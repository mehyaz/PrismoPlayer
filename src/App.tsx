...
import { SkipSegment, Movie, RecentlyWatchedItem, TorrentProgress, SubtitleItem, TorrentItem } from './types';
...
function App() {
  ...
  // Moved handleHeroSearch BELOW handleTorrentStream because it uses it
  ...
  const handleFindTorrent = useCallback(async (movieOverride?: Movie) => {
    ...
    try {
      if (window.ipcRenderer) {
          ...
          const query = `${movieToSearch.title} ${movieToSearch.year || ''}`;
          const results = await window.ipcRenderer.invoke('list-torrents', query);
